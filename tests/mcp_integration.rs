...
/// Mock transport for testing MCP messages
struct MockTransport {
    pub sent_messages: Vec<Message>,
    pub receive_queue: mpsc::UnboundedReceiver<Message>,
    pub send_queue: mpsc::UnboundedSender<Message>,
}

impl MockTransport {
    fn new() -> (Self, mpsc::UnboundedSender<Message>) {
        let (send_tx, receive_rx) = mpsc::unbounded_channel();
        let (send_tx_clone, _) = mpsc::unbounded_channel::<Message>();
        
        (
            Self {
                sent_messages: Vec::new(),
                receive_queue: receive_rx,
                send_queue: send_tx_clone,
            },
            send_tx,
        )
    }
...
async fn test_mcp_initialize_response() -> Result<()> {
    ...
    let mut handler = McpHandler::new(&mut tfmcp);
    
    // Create mock transport
    let (mut mock_transport, message_sender) = MockTransport::new();
    
    // Send initialize request
    let initialize_request = Message::Request {
    ...
    message_sender.send(initialize_request)?;
    
    // Create a simple transport stub for testing
    // Note: This is a simplified test - in a real scenario, you'd need to properly implement the transport
...
async fn test_mcp_tools_list() -> Result<()> {
    ...
    let temp_dir_str = temp_dir.path().to_string_lossy().to_string();
    let mut tfmcp = TfMcp::new(None, Some(temp_dir_str))?;
    let handler = McpHandler::new(&mut tfmcp);
...
async fn test_cache_manager_compilation() -> Result<()> {
    ...
    cache_manager.documentation_cache.set("test_key".to_string(), "test_value".to_string()).await;
    let retrieved = cache_manager.documentation_cache.get("test_key").await;
    assert_eq!(retrieved, Some("test_value".to_string()));
    
    // Test providers cache
    cache_manager.providers_cache.set("provider_key".to_string(), "provider_data".to_string()).await;
