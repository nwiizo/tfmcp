...
mod compilation_tests {
    ...
    fn test_cache_manager_fields() {
        ...
        assert_eq!(std::any::type_name_of_val(&cache_manager.documentation_cache), 
                   std::any::type_name::<tfmcp::registry::cache::DocumentationCache>());
        assert_eq!(std::any::type_name_of_val(&cache_manager.providers_cache), 
...
mod async_compilation_tests {
    ...
    async fn test_async_provider_operations() {
        ...
        let _doc_ids = resolver.resolve_provider_doc_id("test", "hashicorp", "resource", None).await;
    ...
    async fn test_async_registry_client() {
        ...
        let _docs = client.search_docs("test", "hashicorp", "resource", "resources").await;
        
        // Test fallback client methods
        let _fallback_info = fallback_client.get_provider_info("test", None).await;
        let _fallback_docs = fallback_client.search_docs_with_fallback("test", None, "resource", "resources").await;
    }
}