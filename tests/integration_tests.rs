...
async fn test_fallback_client_creation() {
    ...
    assert!(fallback_client.fallback_namespaces.contains(&"hashicorp".to_string()));
    assert!(fallback_client.fallback_namespaces.contains(&"terraform-providers".to_string()));
    assert!(fallback_client.fallback_namespaces.contains(&"community".to_string()));
}
...
async fn test_provider_search_timeout_handling() {
    ...
    assert!(result.is_err(), "Expected timeout error for very short duration");
}
...
async fn test_performance_benchmarks() {
    ...
    assert!(write_duration < Duration::from_millis(100), "Cache writes should be fast");
    ...
    assert!(read_duration < Duration::from_millis(50), "Cache reads should be very fast");
}
